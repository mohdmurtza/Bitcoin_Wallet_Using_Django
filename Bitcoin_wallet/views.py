...
def index(request):
    ...
    if request.method == 'POST':
        ...
        # Make HTTP request to get live Bitcoin price
    ...
    # Make HTTP request to get details about the Bitcoin address
        ...
        final_bal = bal[4].getText().replace(" ", "").rstrip()[:-3].upper()
        transactions = bal[1].getText().replace(" ", "").rstrip()[:-3].upper()
        total_received = bal[2].getText().replace(" ", "").rstrip()[:-3].upper()
        total_sent = bal[3].getText().replace(" ", "").rstrip()[:-3].upper()
    

    # Convert values to float and calculate USD values
        final_bal1_int = float(final_bal)
        total_received1_int = float(total_received)
        total_sent1_int = float(total_received1)
